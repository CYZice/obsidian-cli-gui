...
class CLICommanderSettingTab extends obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new obsidian.Setting(container)
      ...
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.showResultPanel ?? true)
          .onChange(async (value) => {
      ...
      .addText((text) =>
        text
          ...
          .onChange(async (value) => {
            ...
            if (!isNaN(num) && num > 0) {
              this.plugin.settings.maxHistory = num;
              await this.plugin.saveSettings();
            }
          }),
      );
    ...
    new obsidian.Setting(container)
      ...
      .addToggle((toggle) =>
        toggle
          .setValue(this.plugin.settings.moveConflictRename ?? true)
          .onChange(async (value) => {
      ...
      new obsidian.Setting(container)
        ...
        .addDropdown((dropdown) =>
          dropdown
            ...
            .onChange(async (value) => {
              this.plugin.settings.moveConflictSuffix = value;
              await this.plugin.saveSettings();
            }),
        );
    }
    ...
    var hiddenPresets = this.plugin.settings.hiddenBuiltinPresets || [];
    if (hiddenPresets.length) {
      hiddenPresets.forEach((presetId) => {
        var preset = BUILTIN_PRESETS.find((p) => p.id === presetId);
        if (preset) {
          new obsidian.Setting(container)
            .setName(preset.name)
            .setDesc(preset.desc || "")
...
module.exports = { CLICommanderSettingTab };